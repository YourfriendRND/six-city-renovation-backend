...
export default registerAs(NameSpaces.RabbitMQ, () => {
  return validateConfig(RabbitConfigSchema, {
    ...
    url: `amqp://${process.env.RABBITMQ_USER}:${process.env.RABBITMQ_PASSWORD}@${process.env.RABBITMQ_HOST}:${process.env.RABBITMQ_PORT}${process.env.RABBITMQ_VHOST ? '/' + process.env.RABBITMQ_VHOST : ''}`,
  });
});
